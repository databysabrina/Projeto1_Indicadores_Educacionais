...
* **Classificação de Status de Aproveitamento:** estabelecida a partir da taxa de acertos, correlacionada às competências pedagógicas previstas na avaliação.
...
* **Flag de Inconsistência:** Criou-se a coluna ACERTOS_PROFICIENCIA para sinalizar registros onde a nota objetiva divergia do nível atribuído, servindo como uma camada de auditoria de dados para a gestão escolar. Quando há Risco de Dados, a linha é sinalizada como "Analisar Inconsistência".
